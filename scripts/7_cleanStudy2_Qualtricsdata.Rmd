...
127 --> 113 participants = 14 participants did not pass the response bias check

```{r}

progress_filtered$response_check_1[is.na(progress_filtered$response_check_1)] <- "3"

responsecheck_filtered <- progress_filtered %>%
  filter(response_check_1 >= 2.75) %>%
  filter(response_check_1 <= 3.25)
...
exclude_column <- responsecheck_filtered %>%
...
# Filtering out those that get exclude in the above column 
...
113 --> 109 participants = 4 participants failed attention check
