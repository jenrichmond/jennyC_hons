...
missing_per <- perception %>%
  mutate(exclusion = case_when(per_missed == 0 ~ 'Include',
                               TRUE ~ 'Exclude'))

filtered_per <- missing_per %>%
  filter(exclusion == "Include")

missing_att <- attitude %>%
  mutate(exclusion = case_when(att_missed <= 2 ~ 'Include',
                               TRUE ~ 'Exclude'))

filtered_att <- missing_att %>%
  filter(exclusion == "Include")

missing_int <- intention %>%
  mutate(exclusion = case_when(int_missed <= 2 ~ 'Include',
                               TRUE ~ 'Exclude'))

filtered_int <- missing_int %>%
  filter(exclusion == "Include")
...
averaged_att <- filtered_att %>%
  mutate(mean_att = rowMeans(.[2:6], na.rm=TRUE))

averaged_int <- filtered_int %>%
  mutate(mean_int = rowMeans(.[2:5], na.rm=TRUE))

```

JC: Also try all_means <- cbind(filtered_per, averaged_att, averaged_int)

```{r}

att_int_means <- merge(averaged_att, averaged_int, by = 'response_id') %>%
  select (1, 10, 17)

all_means <- merge(att_int_means, filtered_per, by = 'response_id') %>%
  select (1:5) %>%
 relocate(mean_att, .after = perception_1) %>%
   relocate(mean_int, .before = mean_att) 
